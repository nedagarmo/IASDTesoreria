...
switch($action){
	case "create":
		$result = $dao->insert_entries($nombre, $descripcion, $church);
		if($result){
			$jtable_result['Result'] = "OK";
			$jtable_result['Record'] = $dao->get_entries($dao->last_insert_id)->fields;
		} else {
		...
		break;
	case "update":
		$result = $dao->update_entries($id, $nombre, $descripcion, $church);
		if($result){
			$jtable_result['Result'] = "OK";
		} else {
			$jtable_result['Result'] = "ERROR";
	...
	case "search":
		$count_rows = 0;
		if(empty($name)){
			$result = $dao->get_all_entries($church, $jtSorting, $jtStartIndex, $jtPageSize);
			$count_rows = $dao->get_all_count_entries($church)->fields[0];
		}else{
			$result = $dao->get_entries_search($name, $church, $jtSorting, $jtStartIndex, $jtPageSize);
