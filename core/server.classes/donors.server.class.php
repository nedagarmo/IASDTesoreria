...
switch($action){
	case "create":
		$result = $dao->insert_donor($nombre, $identificacion, $church);
		if($result){
			$jtable_result['Result'] = "OK";
			$jtable_result['Record'] = $dao->get_donor($dao->last_insert_id)->fields;
		} else {
		...
		break;
	case "update":
		$result = $dao->update_donor($id, $nombre, $identificacion, $church);
		if($result){
			$jtable_result['Result'] = "OK";
		} else {
			$jtable_result['Result'] = "ERROR";
	...
	case "search":
		$count_rows = 0;
		if(empty($name)){
			$result = $dao->get_all_donors($church, $jtSorting, $jtStartIndex, $jtPageSize);
			$count_rows = $dao->get_all_count_donors($church)->fields[0];
		}else{
			$result = $dao->get_donor_search($name, $church, $jtSorting, $jtStartIndex, $jtPageSize);
