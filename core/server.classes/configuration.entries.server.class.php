...
session_start();
extract($_REQUEST);
include_once(dirname(__FILE__) . '/../data.access.objects/configuration.entries.dao.class.php');

$dao = new configuration_entries();
$jtable_result = array();
...
switch($action){
	case "create":
                ...
                if(($total_percent + $porcentaje) <= 100){
                    $result = $dao->insert_configuration_entries($rubro, $porcentaje, $entrada);
                    if($result){
                            $jtable_result['Result'] = "OK";
                            $jtable_result['Record'] = $dao->get_configuration_entries($dao->last_insert_id)->fields;
                    } else {
                    ...
                    if($total_percent == 100){
                        ...
                        $jtable_result['Result'] = "ERROR";
                        $jtable_result['Message'] = "A esta entrada le resta por se asignado un ".(100 - $total_percent)."%.  Por favor, asigne a este rubro una cantidad menor o igual.";
                    }
                }
		break;
	case "update":
		$result = $dao->update_configuration_entries($id, $rubro, $porcentaje, $entrada);
		if($result){
			$jtable_result['Result'] = "OK";
		} else {
			$jtable_result['Result'] = "ERROR";
		...
		break;
	case "delete":
		$result = $dao->delete_configuration_entries($id);
		if($result){
			$jtable_result['Result'] = "OK";
		} else {
			$jtable_result['Result'] = "ERROR";
		...
		break;
	case "search":
                $result = $dao->get_all_configuration_entries($entry, $jtSorting, $jtStartIndex, $jtPageSize);
                $count_rows = $dao->get_all_count_configuration_entries($entry)->fields[0];
